...
function is_ic_product_search( $query = null ) {
	...
	if ( ! empty( $query->query_vars['ic_search_parsed'] ) || ( ( empty( $query ) && is_search() ) || ( ! empty( $query ) && $query->is_search() ) ) && isset( $_GET['post_type'] ) && is_ic_valid_post_type( $_GET['post_type'] ) ) {
		return true;
	}
...
if ( ! function_exists( 'is_ic_catalog_admin_page' ) ) {
	...
	function is_ic_catalog_admin_page( $check_activation_notice = true ) {
		if ( is_admin() && function_exists( 'get_current_screen' ) ) {
			...
			if ( ( isset( $screen->id ) && ic_string_contains( $screen->id, 'al_product' ) ) || ( $check_activation_notice && is_ic_activation_notice() ) ) {
				return true;
			}
