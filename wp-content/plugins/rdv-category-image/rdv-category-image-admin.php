...
if ( ! class_exists( 'RDV_CT_TAX_META' ) ) {
    ...
    class RDV_CT_TAX_META {
        ...
        */
        public function load_admin_styles_scripts() {
            wp_enqueue_style( 'rdv-category-image-admin-style', plugin_dir_url( __FILE__ ) . 'css/style.css', false, '1.0.0' );
        }
                        ...
                        <form method="post" action="options.php" id="rdv_category_image_options_form">
                            <div class="rdv-admin-fields-row">
                                <?php 
                                    ...
                                    $remove_taxonomies = ['nav_menu','link_category','post_format','author','wp_theme','project_tag','product_type','product_visibility','product_cat','product_tag','product_shipping_class','wp_template_part_area','elementor_library_type','elementor_library_category'];
                                    foreach (get_taxonomies() as $tax) : if (in_array($tax, $remove_taxonomies)) continue; ?>
                                        <div class="rdv-admin-field">
                                            <input class="rdv-admin-checkbox" type="checkbox" id="slug_<?php echo esc_html( $tax ); ?>" name="rdv_category_image_options[rdv_cat_img_checked][<?php echo esc_html( $tax ); ?>]" value="<?php echo esc_html( $tax ); ?>" <?php checked(isset($rdv_category_image_options['rdv_cat_img_checked'][$tax])); ?> /> 
                                        <label class="rdv-admin-label" for="slug_<?php echo esc_html( $tax ); ?>"> <?php echo esc_html( $tax ) ;?></label>
                                        </div>
                                <?php 
                                endforeach;
                                submit_button(); 
        ...
        */
        public function add_rdv_category_image ( $taxonomy ) { ?>
            <div class="form-field term-group cat_rdv">
                <label for="rdv-category-image-id" class="rdv-label"><?php _e('Category Image', 'rdv-category-image'); ?></label>
        ...
        */
        public function update_rdv_category_image ( $term, $taxonomy ) { ?>
            <tr class="form-field term-group-wrap cat_rdv">
                <th scope="row">
                    <label for="rdv-category-image-id" class="rdv-label"><?php _e( 'Edit Category Image', 'rdv-category-image' ); ?></label>
                </th>
                <td>
                    <?php 
    ...
    //$RDV_CT_TAX_META -> init_rdv_category_image();
